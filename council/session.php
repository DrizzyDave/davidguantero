...
if(isset($_POST['login'])){
	...
	$chk = mysqli_query($conn,"SELECT * FROM council WHERE email='$email' and password='$password'");
...
if(isset($_POST['register']))
{
    ...
    $check = mysqli_query($conn,"SELECT * FROM council WHERE email='$email'");
    ...
    {
    	$input = mysqli_query($conn, "INSERT INTO council VALUES('0','$name','$pass','$cn','$email','$address')");
    	if($input == true )
    	{
    		?>
    		<script>
...
if (isset($_SESSION['email']) && isset($_SESSION['password'])) {
	...
	$get = mysqli_query($conn,"SELECT * from council where email='$email' and password='$password'");
	$council = mysqli_fetch_array($get);
}
...
if (isset($_POST['updateProfile'])) {
	...
	mysqli_query($conn,"UPDATE council set name='$name',password='$password',contact='$contact',email='$email',address='$address' where id='$id'");
